...
app.post('/setInsurance', (req, res)=> {
  const iid = req.body.iid;
  const copname = req.body.copname;
  const region = req.body.region;
  ...
  db.query(
    'INSERT INTO hyy_insurance (iid, copname, region, state, city, addressa, addressb, zip) VALUES (?,?,?,?,?,?,?,?)',
    [iid, copname, region, state, city, addressa, addressb, zip],
    (err, result) => {
      if (err) {
        console.log(err)
      } else {
        console.log("set insurance done");
      }
    }
  );
});
...
app.get('/insuranceCheck', (req, res)=> {
  const iid = req.query.iid;

  db.query(
    "SELECT COUNT(*) AS accountno FROM hyy_insurance WHERE iid = ?",
    [iid],
    (err, result) => {
      if (err) {
        console.log(err)
